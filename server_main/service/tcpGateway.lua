...
local function unforward(c)
	if c.agent then
		--skynet.error(string.format("tcpGateway.unforward: close fd=%d", c.fd))
...
local function onTimercheckTimeout()
	...
	for fd, connData in pairs(_connection) do
		if currentTick - connData.tick > _timeoutCheckData.threshold then
			--skynet.error(string.format("tcpGateway.onTimercheckTimeout: close fd=%d", fd))
...
function CMD.initialize(source, type)
	...
	_timeoutCheckData.threshold = tonumber(skynet.getenv("tcpGatewayTimeoutThreshold"))
	...
	_timeoutCheckData.intervalTick = tonumber(skynet.getenv("tcpGatewayTimeoutCheckInterval"))
...
function handler.message(fd, msg, sz)
	...
	if c and c.agent then
		c.tick = skynet.now()
		skynet.redirect(c.agent, 0, "wirePacketStr", 0, msg, sz)
		--skynet.send(c.agent, "wirePacketStr", msg, sz)
	else
...
function handler.connect(fd, addr)
	...
	local agent = skynet.newservice("tcpConnection")
	_forwarding[agent] = c
