...
local function loadProperty()
	local list = {}
	local sql = 'SELECT `ID`, `Gold`, `Discount`, `SendLoveLiness`, `RecvLoveLiness` FROM `QPTreasureDB`.`GameProperty` WHERE `Nullity`=0 AND (`IssueArea` & 6)<>0'
...
local function getTrumpetScore(mysqlConn, userID, memberOrder, trumpetID)
	...
	do
		local sql = string.format(
			"call QPTreasureDB.sp_player_trumpet_score(%d, %d, %d)",
			userID,
			memberOrder,
			trumpetID
		)
...
local function consumeProperty(dbConn, userItem, targetUserID, propertyID, propertyCount)
	...
	local sql = string.format(
		"call QPTreasureDB.sp_player_use_property(%d, %d, %d, %d, %d)",
...
local function prepareTrumpet(trumpetID, userItem, color, msg)
	...
	local sql = string.format(
		"insert into `QPRecordDB`.`TrumpetLog`(`ServerID`, `UserID`, `TrumpetID`, `Color`, `Ctime`, `Msg`) values (%d, %d, %d, %d, now(), '%s')",
		_serverConfig.ServerID,
		userAttr.userID,
		trumpetID,
		color,
...
local function cmd_buyProperty(userItem, propertyID, propertyCount)
	...
	local mysqlConn = addressResolver.getMysqlConnection()
	local sqlTemp = string.format("select totalPay from `QPTreasureDB`.`s_pay` where id=%d", userAttr.userID)
	local rowsTemp = skynet.call(mysqlConn, "lua", "query", sqlTemp)
	if rowsTemp[1]==nil or tonumber(rowsTemp[1].totalPay) < 10000 then
		skynet.send(userAttr.agent, "lua", "forward", 0x010304, {code="RC_MATCH"})
		return
	end
	...
	local sql = string.format(
		"call QPTreasureDB.sp_player_buy_property(%d, %d, %d, %d, %d)",
		_serverConfig.ServerID,
		userAttr.userID,
		propertyID,
		propertyCount,
...
local function cmd_onEventLoginSuccess(data)
	...
	if not data.isAndroid then
		...
		local sql = string.format("SELECT `PropertyID` AS 'propertyID', `PropertyCount` AS 'propertyCount' FROM `QPTreasureDB`.`UserPropertyVendor` WHERE UserID=%d", data.userID)
