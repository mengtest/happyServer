...
local function startGameLogic()
	...
	if androidUserItemSink.onEventStartGameLogic then
		androidUserItemSink.onEventStartGameLogic(_data.status.tableID, _data.status.chairID)
	end
end

local function onResponseLogin(protocalObj)
	if protocalObj.code~="RC_OK" then
		skynet.error(string.format("机器人登录错误: %s", protocalObj.code))
		skynet.exit()
	end
	local nowTime = skynet.now()
	_data.gameStatus = GS_CONST.GAME_STATUS.FREE
	skynet.send(addressResolver.getAddressByServiceName("GS_model_androidManager"), "lua", "setServerUserItem", _data.userID, _data.connectionIdentity.sui, nowTime)
	
	local userAttr = ServerUserItem.getAttribute(_data.connectionIdentity.sui, {"userStatus", "tableID", "chairID"})
