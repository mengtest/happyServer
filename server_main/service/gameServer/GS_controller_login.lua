...
local function getUserInfoFromDb(userID)
	local sql = string.format("call QPAccountsDB.sp_gameserver_login(%d)", userID)
...
local REQUEST = {
	[0x010100] = function(tcpAgent, pbObj, tcpAgentData)
		...
		local retCode, userInfo = queryLoginServer(pbObj.session)
