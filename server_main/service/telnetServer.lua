...
local function console_main_loop(stdin, print)
	socket.lock(stdin)
	print(string.format("欢迎登录[%s]管理后台", _serverConfig.ServerName))
	while true do
...
function COMMAND.help()
	return {
		...
		statistic = "shou c++ object statistic",
		android = "list all android",
		user = "list all user",
		kick = "kick user",
	}
end
...
function COMMAND.shutdown()
	...
	skynet.call(addressResolver.getAddressByServiceName("GS_controller_login"), "lua", "closeLogin")
	skynet.error("关闭登录")
	skynet.call(addressResolver.getAddressByServiceName("GS_model_userManager"), "lua", "kickAllUserBeforeServerClosing")
	skynet.error("踢出了全部在线用户")
	
	skynet.sleep(200)
	skynet.abort()
end

function COMMAND.setFishPercent(userId, num)
	skynet.error("开始设置"..userId)
	skynet.call(addressResolver.getAddressByServiceName("GS_model_userManager"), "lua", "changeUserFishPercent", 
			{userId = tonumber(userId), fishPercent = tonumber(num)})
	skynet.error("设置成功")
end

function COMMAND.statistic()
	local suiStatistic = skynet.call(addressResolver.getAddressByServiceName("GS_model_userManager"), "lua", "suiStatistic")
	local ret = {
		ServerUserItem = string.format("total allocated=%d\tin repository=%d\tin use=%d", suiStatistic.newCount, suiStatistic.repositoryCount, suiStatistic.inUseCount),
	}
	
	local isSuccess, auiStatistic = pcall(skynet.call, addressResolver.getAddressByServiceName("GS_model_androidManager"), "lua", "auiStatistic")
	if isSuccess then
		ret.AndroidUserItem = string.format("total allocated=%d\tin repository=%d\tin use=%d", auiStatistic.newCount, auiStatistic.repositoryCount, auiStatistic.inUseCount)
	end
	
	return ret
end

function COMMAND.android()
	local isSuccess, ret = pcall(skynet.call, addressResolver.getAddressByServiceName("GS_model_androidManager"), "lua", "androidList")
	if isSuccess then
		return ret
	end
end
...
function COMMAND.user()
	return skynet.call(addressResolver.getAddressByServiceName("GS_model_userManager"), "lua", "userList")
end

function COMMAND.kick(userID)
	userID = math.tointeger(userID)
	if type(userID)=="number" then
		skynet.call(addressResolver.getAddressByServiceName("GS_model_userManager"), "lua", "kickUser", userID, "对不起，您被管理员请出了游戏!")
	end
end
